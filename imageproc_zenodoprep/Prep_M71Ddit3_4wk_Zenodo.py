...
last_csv = '/Volumes/Image_Data/M71iCreiGFP_Ddit3cKO_Experiments/Analysis_Glomeruli/Blinded_Assignments_4wk/LookupTable_09_12_21_Output.tsv'
regex = '^MAX.*(WT|Ctrl|Exp).*20x.*.tif$'
file_copy_dir = '/Volumes/Backup_HJS/Papers/UPR_AxonGuidance_2021/Zenodo/Images_20xGlomeruli/4wk_M71_Ddit3'
...
f_list = [os.path.join(subdir, f) for subdir in subdirs for f in os.listdir(subdir) if re.match(regex, f) is not None and re.search('4wk',f) is not None]
...
for gt in ['WT','Ctrl','Exp']:
	...
	for anim_ in anims:
		...
		for lab,fpath_ in zip(['LLateral','RLateral'], files_out_dict[anim_]):
			...
			#Find Configuration
			try:
				config = parsed_entries_dict['4wk_' + anim_ + '_' + lab]['config'] #annoying- had put 4wk in the lookuptable!
				if config == 'none': 
