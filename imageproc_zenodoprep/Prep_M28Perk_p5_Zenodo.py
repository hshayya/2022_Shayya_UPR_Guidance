...
#User Inputs
dir_ = '/Volumes/Image_Data/Mor28iCreiGFP_PerkcKO_Experiments' #some of these old
last_csv = '/Volumes/Image_Data/Mor28iCreiGFP_PerkcKO_Experiments/Analysis_Glomeruli_WTvsCtrl/Blinded_Annotations/LookupTable_05_26_21_Output.tsv'
regex = '^MAX.*(WT|Ctrl|Exp).*20x.*.tif$'
file_copy_dir = '/Volumes/Backup_HJS/Papers/UPR_AxonGuidance_2021/Zenodo/Images_20xGlomeruli/p5_Mor28_Perk'
...
f_list = [os.path.join(subdir, f) for subdir in subdirs for f in os.listdir(subdir) if re.match(regex, f) is not None and re.search('4wk|tdtomOnly|Bundles|GFP|MedialGloms',f) is None]
...
for gt in ['WT','Ctrl','Exp']:
	...
	for slice_,c,t,b in zip(range(out_show.getDimensions()[3]), configs, top_lab, bot_lab):
		...
		#Downsample: otherwise max out memory,...
		imp = imp.resize(int(imp.getDimensions()[0]*0.75), int(imp.getDimensions()[1]*0.75),'bilinear')
		out_ls[slice_] = imp
