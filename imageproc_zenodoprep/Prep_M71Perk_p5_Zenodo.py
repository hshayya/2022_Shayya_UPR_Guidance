...
#User Inputs
dir_ = '/Volumes/LongTerm_Backup/Image_Data/M71iCreiGFP_PerkcKO_Experiments' #some of these old
last_csv = '/Volumes/LongTerm_Backup/Image_Data/M71iCreiGFP_PerkcKO_Experiments/Analysis_Glomeruli_WTvsCtrl/Blinded_Annotation/LookupTable_4-4-21_output.tsv'
regex = '^MAX.*(WT|Ctrl|Exp).*20x.*.tif$'
file_copy_dir = '/Volumes/Backup_HJS/Papers/UPR_AxonGuidance_2021/Zenodo/Images_20xGlomeruli/p5_M71_Perk'
...
f_list = [os.path.join(subdir, f) for subdir in subdirs for f in os.listdir(subdir) if re.match(regex, f) is not None and re.search('[24]wk|rednot|RedGreen|redonly|noglom|redbright',f) is None]

#Group files by animal
files_dict = defaultdict(list)
for f in f_list:
	animal = ''.join(re.search('(WT|Ctrl|Exp)_?([0-9]+)?', os.path.split(f)[1]).groups())
	if animal == 'Exp20':
		animal = 'Exp2' #mistake on this label.
	files_dict[animal].append(f)

#Order the files within each animal, inserting blanks in the correct places
glom_order = ['L_?(dorso)?[Ll]ateral|L.tif|L_Zstack','L_?(ventro)?(Medial|me)','R_?(dorso)?[Ll]ateral|R.tif|R_Zstack','R_?(Medial|me)']
...
for gt in ['WT','Ctrl','Exp']:
	...
	for anim_ in anims:
		...
		for lab,fpath_ in zip(['LLateral','LMedial','RLateral','RMedial'], files_out_dict[anim_]):
	...
	out = montage_.makeMontage2(out_show, 4, int(len(out_ls)/4), 1, 1, len(out_ls), 1, 0, False)
	out.setTitle(gt)
	out.show()