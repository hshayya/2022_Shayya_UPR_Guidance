...
#User Inputs
dir_ = '/Volumes/Image_Data/M71iCreiGFP_Hspa5cKO_Experiments' 
last_csv = '/Volumes/Backup_HJS/Papers/UPR_AxonGuidance_2021/Zenodo/Images_20xGlomeruli/p5_M71_Hspa5/LookupTable_8-7-22_Output.tsv'
regex = '^MAX.*(WT|Ctrl).*20x.*.tif$'
file_copy_dir = '/Volumes/Backup_HJS/Papers/UPR_AxonGuidance_2021/Zenodo/Images_20xGlomeruli/p5_M71_Hspa5'
fix_mappings = {'WT':'WT','Ctrl':'Het'} #on my filesystem WT = WT, Ctrl = Het. Update names so images posted to Zenodo use same names as we do in paper.
...
for f in f_list:
	animal = re.search('(WT|Ctrl)[0-9]+', os.path.split(f)[1]).group()
